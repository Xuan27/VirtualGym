...
public class PathFollower : MonoBehaviour
{
    ...
    void CheckNode()
    {
        ...
        {
            Error = Vector3.Distance(ErrorNode[CurrentNode + 1].transform.position, DraggingObject.transform.position);
            Angle = Vector3.Angle(DraggingObject.transform.position, ErrorNode[CurrentNode + 1].transform.position);
        }
        string objPosition;
        string nodePosition;
        string windowError;
        ...
        {
            objPosition = DraggingObject.transform.position.x + ", " + DraggingObject.transform.position.y + " , " + DraggingObject.transform.position.z + " , " + HoloToolkit.Unity.InputModule.CollisionBound.Instance.FinalCount + System.Environment.NewLine;
            nodePosition = CurrentPositionHolder.x + ", " + (CurrentPositionHolder.y - 0.5).ToString() + ", " + CurrentPositionHolder.z + " , " + HoloToolkit.Unity.InputModule.CollisionBound.Instance.FinalCount + System.Environment.NewLine;
            windowError = WindowErrorBound.Instance.FrameErrorValue.ToString() + ", " + HoloToolkit.Unity.InputModule.CollisionBound.Instance.FinalCount + System.Environment.NewLine;
            objError = Error.ToString() + " , " + HoloToolkit.Unity.InputModule.CollisionBound.Instance.FinalCount + System.Environment.NewLine;
    ...
    void Update()
    {
        MoveSpeedForward = AzureServices.instance.ForwardSpeed;
        MoveSpeedBackward = AzureServices.instance.BackwardSpeed;
        Vector3 lastPosition = new Vector3( 2,Player.transform.position.y, Player.transform.position.z);
        ...
        {
            if (line % 2 == 1)
                frameYPosition = (float)(Player.transform.position.y + 0.5);
            else
                frameYPosition = (float)(Player.transform.position.y - 0.5);

            framePosition0 = new Vector3(Player.transform.position.x, frameYPosition, Player.transform.position.z);
            framePosition1 = new Vector3(1, frameYPosition, Player.transform.position.z);
        ...
        { 
            resetTraining();
            AzureServices.instance.azureStatusText.text = ConditionalTraining.Instance.TrainingType + " Completed ...";
    ...
    async void createUploadList()
    {
       ...
       await AzureServices.instance.UploadTrainingInfo(ConditionalTraining.Instance.TrainingType);
    }
